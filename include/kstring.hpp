 ...
 *       ModifyBy:  Jerry Shi (), jerryshi0110@gmail.com
 *        Company:  B5M.com
 *
 * =====================================================================================
 */
...
#define CHECK_NULL(mem) {if(!mem)return;}
    ...
    void reserve(uint32_t len)
    {
        ...
        assert(s > sizeof(uint32_t)*3);
        uint32_t* m = (uint32_t*)malloc(s);
        memset(m, 0, s);
        if (mem_)
        {
            assert(total_bytes_() > sizeof(uint32_t)*3);
            assert(total_bytes_() < s);
